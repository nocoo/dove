...
/**
 * Health badge. Three-state:
 *   - healthy (green check)
 *   - degraded (amber) when configValid but reachable=false
 *   - unhealthy (red) when configValid=false
 */
...
export default function ProvidersPage() {
  ...
  const [healthLoadingById, setHealthLoadingById] = useState<
  ...
  const fetchHealth = useCallback(async (id: string) => {
    ...
    try {
      ...
      // leave as unknown; user can retry
    } finally {
  ...
  // Fire-and-forget health fetch for each provider once we have the list.
  useEffect(() => {
    if (!providers) return;
  ...
  return (
    <AppShell breadcrumbs={[{ label: "Providers", href: "/providers" }]}>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Outbound email backends. Each project picks one; health here
              is a cheap probe, not a guarantee of delivery.
            </p>
          </div>
