...
export class CloudflareProvider implements EmailProvider {
	...
	private async acquireIdempotencySlot(key: string): Promise<void> {
		...
		const result = await db
			.prepare(
				"INSERT OR IGNORE INTO cf_email_idempotency (id, status, created_at) VALUES (?, 'pending', ?)",
			)
		...
		const reclaim = await db
			.prepare(
				"UPDATE cf_email_idempotency SET status = 'pending', created_at = ?, completed_at = NULL WHERE id = ? AND status = 'failed'",
			)
			.bind(new Date().toISOString(), key)
			.run();
...
function createMimeMessage(params: SendParams): { from: string; to: string } {
	...
	const EM = EmailMessage as unknown as {
		new (from: string, to: string, raw: ReadableStream | string): { from: string; to: string };
	};
	return new EM(fromAddr, params.to, rawEmail);
}
