...
describe("CloudflareProvider", () => {
	...
	describe("send without D1", () => {
		...
		test("RFC 2047-encodes non-ASCII Subject and From name; leaves ASCII as-is", async () => {
			...
			const subjectB64 = m?.[1];
			expect(subjectB64).toBeDefined();
			if (!subjectB64) throw new Error("expected subject base64");
			const decoded = new TextDecoder().decode(
				Uint8Array.from(atob(subjectB64), (c) => c.charCodeAt(0)),
			);
		...
		test("DEFENSE: CR/LF in Subject triggers RFC 2047 base64 (blocks header injection)", async () => {
			...
			if (!decodedB64) throw new Error("expected subject base64");
		...
		test("raw MIME has CRLF line endings AND a properly-closed multipart boundary", async () => {
			...
			expect(msg.raw).toMatch(
				/Content-Type: multipart\/alternative; boundary="----cf[a-f0-9]+"\r\n/,
			);
