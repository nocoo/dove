...
describe("ResendProvider", () => {
  ...
  describe("send", () => {
    ...
    test("retries on network error", async () => {
      ...
      const call1 = mockFetch.mock.calls[0] as unknown as [string, { headers: Record<string, string>; body: string }];
      const call2 = mockFetch.mock.calls[1] as unknown as [string, { headers: Record<string, string>; body: string }];
    ...
    test("throws after max retries on persistent 5xx", async () => {
      ...
      const urls = mockFetch.mock.calls.map((c) => (c as unknown as [string])[0]);
      expect(new Set(urls).size).toBe(1);
      const bodies = mockFetch.mock.calls.map((c) => ((c as unknown as [string, { body: string }])[1]).body);
