...
describe("markdownToHtml", () => {
	...
	test("PRESERVES safe schemes (https, mailto, relative paths)", async () => {
		...
		expect(await markdownToHtml("[b](mailto:a@b.c)")).toContain('href="mailto:a@b.c"');
		expect(await markdownToHtml("[c](/relative/path)")).toContain('href="/relative/path"');
	});
...
describe("renderTemplate", () => {
	const schema: TemplateVariable[] = [{ name: "name", type: "string", required: true }];
	...
	test("escapes HTML in variables for XSS prevention", async () => {
		const result = await renderTemplate("Hello {{name}}", "Content for {{name}}", schema, {
	...
	test("throws on missing required variable", async () => {
		await expect(renderTemplate("Hello", "Body", schema, {})).rejects.toThrow(
