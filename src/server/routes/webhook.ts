...
import { getRecipient, getRecipientByEmail, type Recipient } from "../lib/db/recipients";
import {
	createSendLog,
	...
	try {
		...
		if (!authHeader?.startsWith("Bearer ")) {
			return logAndRespond(
				401,
				errorJson("auth_missing", "Missing Authorization header"),
				"auth_missing",
			);
		}
		...
		if (!project || !constantTimeEqual(project.webhook_token, token)) {
			return logAndRespond(
				403,
				errorJson("auth_invalid", "Invalid token or project not found"),
				"auth_invalid",
			);
		}
		...
		if (!parsed.success) {
			return logAndRespond(
				400,
				errorJson("body_invalid", "Request body validation failed"),
				"body_invalid",
		...
		if (idempotencyKey) {
			...
			if (existingSendLog) {
				...
				if (existingSendLog.status === "sending") {
					return logAndRespond(
						409,
						errorJson("send_in_progress", "This request is already being processed"),
						"send_in_progress",
					);
				}
			}
		...
		if (!recipient) {
			return logAndRespond(
				404,
				errorJson("recipient_not_found", "Recipient not found in project whitelist"),
				"recipient_not_found",
			);
		}
		...
		if (!template) {
			return logAndRespond(
				404,
				errorJson("template_not_found", "Template slug not found in project"),
				"template_not_found",
			);
		}
		...
		try {
			...
			return logAndRespond(
				422,
				errorJson("variables_invalid", message),
				"variables_invalid",
				message,
			);
		}
		...
		if (!rateLimitResult.allowed) {
			...
			return logAndRespond(
				...
				{
					error: {
						code: "rate_limit_address",
						message: `Rate limit: wait ${retryAfter} seconds before sending to this address again`,
		...
		try {
			...
			try {
				if (project.provider_id) {
					...
					if (!record) {
						...
						return logAndRespond(
							500,
							errorJson("provider_not_found", "Configured email provider not found"),
							"provider_not_found",
						);
					}
				...
				return logAndRespond(
					500,
					errorJson("provider_config_invalid", message),
			...
			try {
				...
				return logAndRespond(
					500,
					errorJson("internal_error", "Email sender not configured"),
					"internal_error",
				);
			}
		...
		return logAndRespond(
			500,
			errorJson("internal_error", "Unexpected server error"),
			"internal_error",
