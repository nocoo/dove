...
export type { EmailProviderRecord, EmailProviderType } from "@/lib/types/email-provider";

import type { EmailProviderRecord, EmailProviderType } from "@/lib/types/email-provider";

/**
 * List all providers, ordered by creation date descending.
 */
export async function listEmailProviders(db: D1Database): Promise<EmailProviderRecord[]> {
	return query<EmailProviderRecord>(db, "SELECT * FROM email_providers ORDER BY created_at DESC");
}
...
export async function getEmailProvider(
	...
	return queryOne<EmailProviderRecord>(db, "SELECT * FROM email_providers WHERE id = ?", [id]);
}
 ...
 */
export async function deleteEmailProvider(db: D1Database, id: string): Promise<boolean> {
	const existing = await getEmailProvider(db, id);
 ...
 */
export async function countProjectsByProvider(db: D1Database, providerId: string): Promise<number> {
