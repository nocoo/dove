 ...
 */
export async function listRecipients(db: D1Database, projectId: string): Promise<Recipient[]> {
 ...
 */
export async function getRecipient(db: D1Database, id: string): Promise<Recipient | null> {
...
export async function getRecipientByEmail(
	...
	return queryOne<Recipient>(db, "SELECT * FROM recipients WHERE project_id = ? AND email = ?", [
		projectId,
...
export async function updateRecipient(
	...
	await execute(db, "UPDATE recipients SET name = ?, email = ? WHERE id = ?", [name, email, id]);
 ...
 */
export async function deleteRecipient(db: D1Database, id: string): Promise<boolean> {
	const existing = await getRecipient(db, id);
