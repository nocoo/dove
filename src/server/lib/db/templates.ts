 ...
 */
export async function listTemplates(db: D1Database, projectId: string): Promise<Template[]> {
...
export async function listAllTemplates(db: D1Database): Promise<Template[]> {
	return query<Template>(db, "SELECT * FROM templates ORDER BY created_at DESC");
}
...
export async function getTemplateBySlug(
	...
	return queryOne<Template>(db, "SELECT * FROM templates WHERE project_id = ? AND slug = ?", [
		projectId,
		slug,
	]);
}
