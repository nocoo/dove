...
const mockGetRecipientByEmail = vi.fn(() =>
	Promise.resolve(sampleRecipient as typeof sampleRecipient | null),
);
...
const mockCheckQuota = vi.fn(() =>
	Promise.resolve({ allowed: true } as { allowed: boolean; error_code?: string }),
);
const mockRenderTemplate = vi.fn(() =>
	Promise.resolve({ subject: "Hello World", html: "<p>Welcome!</p>" }),
);
const mockAcquireAddressLock = vi.fn<
	() => Promise<{ allowed: boolean; lock_token?: string; retry_after_seconds?: number }>
>(() => Promise.resolve({ allowed: true, lock_token: "test-lock-token" }));
...
function sendRequest(body: Record<string, unknown>, token = sampleProject.webhook_token) {
	const { req } = createApp();
...
describe("webhook route handlers", () => {
	...
	describe("GET /:projectId/templates", () => {
		test("returns template list with whitelisted fields ONLY (no internal id, no body_markdown)", async () => {
			...
			if (!t) throw new Error("expected template entry");
	...
	describe("POST /:projectId/send", () => {
		...
		test("fresh idempotent send (no existing log) computes payload_hash and creates new log", async () => {
			...
			const createArgs = mockCreateSendLog.mock.calls[0] as unknown as [
		...
		test("returns 401 without auth", async () => {
			...
			const wlArgs = mockCreateWebhookLog.mock.calls[0] as unknown as [
				unknown,
				Record<string, unknown>,
			];
		...
		test("returns 403 with invalid token", async () => {
			const res = await sendRequest({ template: "welcome", to: "user@example.com" }, "wrong_token");
			expect(res.status).toBe(403);
		...
		test("idempotency key is INVARIANT to variable insertion order (covers sort comparator)", async () => {
			...
			const h1 = (
				mockCreateSendLog.mock.calls.at(-2) as unknown as [unknown, { payload_hash: string }]
			)[1].payload_hash;
			const h2 = (
				mockCreateSendLog.mock.calls.at(-1) as unknown as [unknown, { payload_hash: string }]
			)[1].payload_hash;
			expect(h1).toBe(h2);
		...
		test("returns 429 with quota_daily_exceeded fallback when error_code missing", async () => {
			...
			mockCheckQuota.mockImplementation(() => Promise.resolve({ allowed: false }));
			const res = await sendRequest({
				template: "welcome",
				to: "user@example.com",
			});
		...
		test("creates webhook log on every response", async () => {
			...
			// and >= 0.
			const args = mockCreateWebhookLog.mock.calls[0] as unknown as [
				unknown,
				Record<string, unknown>,
			];
			const entry = args[1];
		...
		describe("Step 5 recipient — allow_unknown_recipients", () => {
			...
			test("flag ON + valid email: accepts and SKIPS whitelist lookup", async () => {
				...
				expect(mockProviderSend).toHaveBeenCalledTimes(1);
				const sendArgs = (
					mockProviderSend.mock.calls[0] as unknown as [Record<string, unknown>]
				)[0];
