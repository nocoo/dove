...
function createMockDB(opts: { allResults?: unknown[]; firstResult?: unknown } = {}) {
...
describe("stats route handlers", () => {
	...
	test("GET / aggregates daily/monthly sends across multiple projects", async () => {
		...
		if (!failedSql) throw new Error("expected failed-count SQL");
	...
	test("GET /charts returns 30-day chart data", async () => {
		...
		const dateBoundedSqls = sqls.filter(
			(s) => /sent_at|created_at/i.test(s) && /date\s*\(/i.test(s),
		);
