...
function requireParam(p: string[], i: number): string {
	const v = p[i];
	if (v === undefined) throw new Error(`expected bind param at index ${i}`);
	return v;
}
...
function createMockDB() {
	const locks = new Map<
		string,
		{ blocked_until: string; lock_token: string; created_at: string }
	>();
				...
				return {
					run: vi.fn(() => {
						if (/INSERT OR IGNORE/i.test(sql)) {
							const key = makeKey(requireParam(p, 0), requireParam(p, 1));
							if (locks.has(key)) {
								return Promise.resolve({ meta: { changes: 0 }, success: true });
							}
							locks.set(key, {
								blocked_until: requireParam(p, 2),
								lock_token: requireParam(p, 3),
								created_at: requireParam(p, 4),
							});
						...
						if (/UPDATE rate_limit_locks/i.test(sql)) {
							...
							const key = makeKey(requireParam(p, 3), requireParam(p, 4));
							const existing = locks.get(key);
							if (existing && existing.blocked_until <= requireParam(p, 5)) {
								locks.set(key, {
									blocked_until: requireParam(p, 0),
									lock_token: requireParam(p, 1),
									created_at: requireParam(p, 2),
								});
								return Promise.resolve({ meta: { changes: 1 }, success: true });
							}
							return Promise.resolve({ meta: { changes: 0 }, success: true });
						...
						if (/DELETE FROM rate_limit_locks/i.test(sql)) {
							...
							const key = makeKey(requireParam(p, 0), requireParam(p, 1));
							const existing = locks.get(key);
							if (existing && existing.lock_token === requireParam(p, 2)) {
								locks.delete(key);
								return Promise.resolve({ meta: { changes: 1 }, success: true });
							}
							return Promise.resolve({ meta: { changes: 0 }, success: true });
...
describe("rate-limit: acquireAddressLock", () => {
	...
	test("custom window duration is respected", async () => {
		...
		if (!lock) throw new Error("expected lock row");
...
describe("rate-limit: releaseAddressLock", () => {
	test("removes lock with matching token", async () => {
		...
		if (!token) throw new Error("expected lock_token");
