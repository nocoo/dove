...
// Mock D1Database
function createMockDb(options: { queryResults?: Template[]; firstResult?: Template | null }) {
	const mockStmt = {
...
describe("Templates CRUD (native D1)", () => {
	...
	describe("getTemplateBySlug", () => {
		test("returns template when slug matches AND pins WHERE project_id + slug binds", async () => {
			...
			const result = await getTemplateBySlug(mockDb, template.project_id, template.slug);
	...
	describe("updateTemplate", () => {
		...
		test("updates variables array", async () => {
			...
			const newVars: TemplateVariable[] = [{ name: "new_var", type: "boolean", required: false }];
			const result = await updateTemplate(mockDb, existing.id, {
				variables: newVars,
			});

			expect(result).not.toBeNull();
			if (!result) throw new Error("expected updated template");
			const parsed = JSON.parse(result.variables) as TemplateVariable[];
