...
describe("templates route handlers", () => {
  ...
  test("POST / creates template 201", async () => {
    ...
    const body = (await res.json()) as { id: string; project_id: string; slug: string; name: string; subject: string; body_markdown: string };
    ...
    const dbThatThrowsUnique = {
      prepare: vi.fn(() => ({
        bind: vi.fn(() => ({
          ...
          first: vi.fn(() => Promise.resolve(null)),
          run: vi.fn(() => Promise.reject(new Error("D1_ERROR: UNIQUE constraint failed: templates.slug"))),
        })),
      })),
    } as unknown as D1Database;
  ...
  test("DELETE /:id returns 204 AND issues DELETE SQL with bound id (silent-no-op defense)", async () => {
    ...
    const { req } = createApp(
      createMockDB({ firstResult: sampleTemplate, sqlSeen, bindsSeen }),
    );
  ...
  test("POST /:id/test-send returns 500 when configured provider missing", async () => {
    ...
    const { req } = createApp(createMockDB({ firstResults: [sampleTemplate, projectWithProvider, null] }));
    const res = await req("/tpl_001/test-send", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ to: "user@example.com", variables: { name: "A" } }),
