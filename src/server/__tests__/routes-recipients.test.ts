...
describe("recipients route handlers", () => {
  ...
  test("POST / creates recipient 201", async () => {
    ...
    const body = (await res.json()) as { id: string; project_id: string; name: string; email: string };
    ...
    const dbThatThrowsUnique = {
      prepare: vi.fn(() => ({
        bind: vi.fn(() => ({
          ...
          first: vi.fn(() => Promise.resolve(null)),
          run: vi.fn(() => Promise.reject(new Error("D1_ERROR: UNIQUE constraint failed: recipients.email"))),
        })),
      })),
    } as unknown as D1Database;
  ...
  test("DELETE /:id returns 204 on success AND issues DELETE SQL with bound id (defense against silent-no-op regression)", async () => {
    ...
    const { req } = createApp(
      createMockDB({ firstResult: sampleRecipient, sqlSeen, bindsSeen }),
    );
