...
function providerMessageLabel(log: { provider_type: SendLog["provider_type"] }): string {
	...
	const fetchData = useCallback(async () => {
		try {
			...
			if (logsRes.ok) {
				...
				setLogs(data);
				setTotal(
					data.length < PAGE_SIZE ? (page - 1) * PAGE_SIZE + data.length : (page + 1) * PAGE_SIZE,
				);
			}
	...
	return (
		<div className="flex flex-col gap-6">
			<div>
				...
				<p className="text-sm text-muted-foreground mt-1">Authoritative email send history.</p>
			</div>
			...
			{loading && !logs ? (
				...
				<div className="relative">
					...
					{/* Custom flex list — expandable rows + responsive stack (not a data grid) */}
					<div>
					...
					{totalPages > 1 && (
						<div className="flex items-center justify-between pt-3">
							...
							<div className="flex items-center gap-1">
								...
								{generatePageNumbers(page, totalPages).map((p, i, arr) => {
									if (p === "...") {
										const key = i < arr.length / 2 ? "ellipsis-start" : "ellipsis-end";
										return (
											<span key={key} className="px-1 text-xs text-muted-foreground">
												...
											</span>
										);
									}
