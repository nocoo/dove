...
interface TestBlock { file: string; name: string; body: string; line: number; describePath: string }
...
function computeDescribePath(src: string, testIdx: number): string {
  ...
  while (i < testIdx) {
    ...
    if (c === '"' || c === "'" || c === "`") {
      const q = c; i++;
      while (i < testIdx && src[i] !== q) { if (src[i] === "\\") i++; i++; }
      i++;
      continue;
    }
    if (c === "/" && src[i + 1] === "/") {
...
for (const [file, m] of dupByFile) for (const [name, c] of m) if (c > 1) dups.push({ file, name, count: c });
