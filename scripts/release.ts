#!/usr/bin/env bun
/**
 * Automated release script for dove.
 *
 * Bumps version in package.json, syncs lockfile, generates CHANGELOG
 * entries from conventional commits, verifies no stale version strings,
...
// ---------------------------------------------------------------------------
// Version targets (single-package — just package.json)
// ---------------------------------------------------------------------------
...
async function main(): Promise<void> {
  ...
  console.log(
    `📝 Phase 1: Updating version in ${VERSION_TARGETS.length} file(s)...\n`,
  );
