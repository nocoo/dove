...
function extractTestBodies(file: string): string[] {
  ...
  const out: string[] = [];
  const re = /\b(?:test|it)\(\s*(?:"[^"]+"|'[^']+'|`[^`]+`)\s*,\s*(?:async\s*)?(?:\(\s*\)|[a-zA-Z_$]+)\s*=>\s*\{/g;
...
function auditQuality(): { total: number; noAssert: number; weak: number; assertionsPerTest: number; statusOnly: number; calledTimesWithoutArgs: number; rejectsWithoutSideEffectCheck: number } {
  let total = 0, noAssert = 0, weak = 0, assertions = 0, statusOnly = 0, calledTimesWithoutArgs = 0, rejectsWithoutSideEffectCheck = 0;
  ...
  const TAUTOLOGY = /\bexpect\(\s*(true|false|null|undefined|\d+|"[^"]*"|'[^']*'|`[^`]*`)\s*\)\s*\.\s*toBe\(\s*\1\s*\)/;
  ...
  const HAS_BODY_ASSERT = /\bexpect\([^)]*\b(body|json|text)\b|\.toEqual\(|\.toMatch\(|\.toContain\(|\.toHaveProperty\(|\.toHaveBeen/;
  for (const root of ["src/__tests__", "src/server/__tests__"]) {
    ...
    for (const f of walk(root)) {
      for (const body of extractTestBodies(f)) {
        ...
        const hasArgsCheck = /\.mock\.calls[.\[]|\.toHaveBeenCalledWith\s*\(|\.toHaveBeenLastCalledWith\s*\(|\.toHaveBeenNthCalledWith\s*\(/.test(body);
        ...
        const hasRejects = /\bexpect\([^)]*\)\s*\.\s*rejects\s*\.|\bawait\s+expect\([^)]*\)\s*\.\s*rejects\s*\./.test(body);
        const hasAnyCallCheck = /\.toHaveBeenCalled(?:Times|With|LastWith|NthWith)?\s*\(|\.mock\.calls[.\[]/.test(body);
        if (hasRejects && !hasAnyCallCheck) rejectsWithoutSideEffectCheck++;
      }
    }
  }
  return { total, noAssert, weak, assertionsPerTest: total ? assertions / total : 0, statusOnly, calledTimesWithoutArgs, rejectsWithoutSideEffectCheck };
}
...
function auditSqlWindowGaps(): { total: number; unpinned: number; details: string[] } {
  const FRAG_RE = /(strftime\('[^']+'(?:\s*,\s*'[^']*')?\)|date\('now'(?:\s*,\s*'[+-]\d+\s+\w+')?\))/g;
  ...
  for (const root of sutRoots) {
    ...
    for (const f of walkAll(root)) {
      ...
      let m: RegExpExecArray | null;
      while ((m = FRAG_RE.exec(src))) fragments.add(m[1]);
    }
  }
  ...
  for (const frag of fragments) {
    ...
    if (fnMatch && argMatch && new RegExp(`${fnMatch[1]}\\s*\\(\\s*\\\\?'?${argMatch[1].replace(/[+\-]/g, "[+\\-]")}`).test(testCorpus)) continue;
    unpinned.push(frag);
  }
...
function runOnce(withCoverage: boolean): RunResult {
  ...
  if (mFail) {
    failed = Number(mFail[1]); passed = Number(mFail[2]); total = Number(mFail[3]);
  } else if (mPass) {
    passed = Number(mPass[1]); total = Number(mPass[2]);
  }
  // All files       |   95.5  |   88.3 |  96.1 |  95.5 |
  let lineCov = 0, funcCov = 0, branchCov = 0;
  if (withCoverage) {
    const cm = out.match(/All files\s*\|\s*([\d.]+)\s*\|\s*([\d.]+)\s*\|\s*([\d.]+)\s*\|\s*([\d.]+)/);
    if (cm) {
...
console.log(`audit: ${q.total} tests, ${q.noAssert} no-assert, ${q.weak} weak-smoke, ${q.statusOnly} status-only, ${q.calledTimesWithoutArgs} called-times-no-args, ${q.rejectsWithoutSideEffectCheck} rejects-no-side-effect, ${q.assertionsPerTest.toFixed(2)} expects/test`);
...
if (!statusOnlyOk) {
  console.error(`FAIL: status-only test gate (${q.statusOnly} tests assert ONLY .status with no body shape — these pass even if response body is wrong)`);
  process.exit(4);
}
if (!calledTimesOk) {
  console.error(`FAIL: called-times-no-args gate (${q.calledTimesWithoutArgs} tests assert mock.toHaveBeenCalledTimes(N) but never inspect what the mock was called WITH — these pass even if the right mock is called with the wrong arguments)`);
  process.exit(5);
}
if (!rejectsOk) {
  console.error(`FAIL: rejects-no-side-effect gate (${q.rejectsWithoutSideEffectCheck} tests assert .rejects.* but never verify side-effect mocks at all — these pass even if the SUT performed an unintended INSERT/POST/KV-write before throwing)`);
  process.exit(6);
}
if (sw.unpinned > 0) {
  console.error(`FAIL: sql-window-unpinned gate (${sw.unpinned} SUT date-fragments are not referenced by any test: ${sw.details.join(", ")} — mocks are time-blind so a regression that changes the SQL window silently passes when the response shape is the only assertion; pin the fragment in the corresponding test via expect(sql).toMatch(/.../))`);
  process.exit(7);
}
if (!allPassed) {
