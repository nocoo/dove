...
| **L2** | ✅ True HTTP | Real HTTP calls to running dev server on port 17034, connecting to `dove-db-test` via **separate test Worker** |
...
**Files**:
- `scripts/run-e2e.ts` — Rewrite to auto-start/stop dev server on port 17034
...
AFTER (target):
  run-e2e.ts → spawn `next dev --port 17034` with E2E env
  test → fetch("http://localhost:17034/api/...") → real HTTP → real D1 (test)
...
# Port — do NOT use "bun run dev", use "next dev --port 17034" directly
PORT=17034
```
...
4. **Clean `.next/dev/lock`** — Next.js prevents parallel dev server instances; stale lock from port 7034 blocks port 17034 startup (backy lesson)
5. Spawn `next dev --port 17034` directly (NOT `bun run dev` which hardcodes port 7034)
6. Wait for server ready (poll `http://localhost:17034/api/live`)
...
- Export `BASE = "http://localhost:17034"`
...
> **Port note**: The `dev` script in `package.json` hardcodes `--port 7034`. Playwright's `webServer.command` must call `next dev --port 27034` directly to control the port.
>
...
**Verification**: Run `bun run test:e2e:bdd` — Playwright reads `playwright.config.ts`, starts webServer on 27034, runs all specs.
...
| L2 | `bun run test:e2e:api` | Server starts on 17034, all tests pass via real HTTP against dove-db-test | ✅ 58 tests pass |
| G2 | `bun run gate:security` | osv-scanner + gitleaks both run and pass (hard fail if missing) | ✅ Pass |
| L3 | `bun run test:e2e:bdd` | Playwright runs core flows on 27034 | ✅ 13 tests pass |
...
| Port conflict (7034 vs 17034/27034) | E2E breaks | Scripts use `next dev --port N` directly, never `bun run dev` |
